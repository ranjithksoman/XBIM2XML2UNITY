...
{
    class IFCtoXML_Compiler
    {

        //const string file = "D:/Ani_Thesis/RecentWork_07052017/trialbim_simple.ifc";
        const string file = "D:/Ani_Thesis/RecentWork_07052017/20160414office_model_CV2_fordesign.ifc";
        //const string file = "D:/Ani_Thesis/RecentWork_07052017/wall.ifc";

        //const string file = "D:/THESIS_ANI/RecentWork_07052017/trialbim_simple.ifc";
        ...
        private static void GenerateHierarchy(IIfcObjectDefinition o, int level, Dictionary<string, IfcSpace> spaceidset, Dictionary<string, IfcBuildingStorey> storeyidset, List<XbimShapeInstance> _shapes, int number_of_shapes, Xbim3DModelContext mod_context)
        {
            ...
            xmlWriter.WriteStartElement(objname.ToString()); //start obj
            if (o.GetType().Name == "IfcProject")
            {
                objname = objname + "_ROOT";
            }
            xmlWriter.WriteAttributeString("Name", objname.ToString());
            ...
            if (!(o.GetType().Name == "IfcProject"))
            {
                ...
                /**************************************************************************/
                xmlWriter.WriteStartElement("Translations"); // start translations
                ...
                xmlWriter.WriteEndElement(); //end translations
            ...
            foreach (var i in item)
            {
                ...
                if (spaceidset.ContainsKey(id))
                {
                    ...
                    if (spacenodelelems.Count() > 0)
                    {
                        ...
                        foreach (var sne in spacenodelelems)
                        {
                            ...
                            Console.WriteLine($"{GetIndent(11)}{"-----Vertices for this face: " + si.IfcProductLabel.ToString()}");
                            Console.WriteLine($"{GetIndent(11)}{"-------------------" + vert_locations.Count() + "--------------------"}");
                ...
                else if (storeyidset.ContainsKey(id))
                {
                    ...
                    if (bsnodelelems.Count() > 0)
                    {
                        ...
                        foreach (var bsne in bsnodelelems)
                        {
                            ...
                            //write the name of the shape  with its id
                            xmlWriter.WriteStartElement(name_of_shape); // start shape
                            ...
                            Console.WriteLine($"{GetIndent(11)}{"-----Vertices for Shape [" + si.IfcProductLabel.ToString()+"]:"}");
                            Console.WriteLine($"{GetIndent(11)}{"-------------------" + vert_locations.Count() + "--------------------"}");

                            xmlWriter.WriteStartElement("Vertices"); //start vertices
                            ...
                            foreach (XbimPoint3D v_l in vert_locations)
                            {
                                ...
                                /*******************************************************************************/
                                xmlWriter.WriteStartElement("Index"); //start index
                                ...
                                xmlWriter.WriteEndElement(); //end index
                            ...
                            xmlWriter.WriteEndElement(); //end vertices
                            /***************************************************************************/
                ...
                /***************************************************************************/
                xmlWriter.WriteEndElement();// end obj
            }
        ...
        private static XbimShapeTriangulation getgeometry (XbimShapeInstance shape, Xbim3DModelContext m_context, int entityLabel, int number_of_shapes) // works per shape
        {

            List<int> vert_indices = new List<int>(); //trial - to get all the vertices defining this XbimShapeInstance
            XbimVector3D vert_normals = new XbimVector3D (); //trial - to get all the vertices defining this XbimShapeInstance
            ...
            xmlWriter.WriteStartElement("VertexNormals"); // start normals
            xmlWriter.WriteAttributeString("CreateGameObject", "False"); //working
            xmlWriter.WriteAttributeString("GenerateMesh", "False"); //trial

            foreach (XbimFaceTriangulation f in facesfound) //per face of the shape
            {

                number_of_triangles = f.TriangleCount;

                Console.WriteLine($"{GetIndent(13)}{"---------------------------------------------------------\n"}");

                Console.WriteLine($"{GetIndent(13)}{"  ------Triangle count on face ["  + face_index.ToString() + "] is : " + number_of_triangles}");
                ...
                var faceindices = f.Indices;
                Console.WriteLine("\n" + GetIndent(15) + "VertexIndices for this face: " + "{0}", string.Join(", ", faceindices));

                //IEnumerable<int> faceindices_distinct =  faceindices.ToList().Distinct();
                
                vert_normals = (normals[0].Normal);
                
                /******************only for cmd display*******************/
                XbimPackedNormal n = normals[0];
                
                Console.WriteLine($"{"\n"}{GetIndent(15)}{"Face[" + face_index.ToString() + "] Normals: "}{n.Normal.X + ","}{n.Normal.Y + ","}{n.Normal.Z}");
                Console.WriteLine($"{GetIndent(15)}{"---------------------------------------------\n"}");

                /*********************************Trial // normals ************************************************************************/

                xmlWriter.WriteStartElement("V_Normals"); //normal node start
                xmlWriter.WriteAttributeString("CreateGameObject", "False"); //working
                xmlWriter.WriteAttributeString("GenerateMesh", "False"); //working
                xmlWriter.WriteAttributeString("VerticesWithThisNormal" , faceindices.Count().ToString());

                xmlWriter.WriteElementString("nX", vert_normals.X.ToString());
                xmlWriter.WriteElementString("nY", vert_normals.Y.ToString());
                xmlWriter.WriteElementString("nZ", vert_normals.Z.ToString());

                xmlWriter.WriteEndElement(); //end normals

                composetrianglesets(f, mesh,vert_indices); // retirieve per-triangle inforamtion to build the mesh
            ...
            /**********************************************************VERT_INDICES******************************************************************/
            Console.WriteLine($"{"\n"}{GetIndent(11)}{"=========================================================================================================="}");
            Console.WriteLine($"{GetIndent(11)}{"Vertex_indices for shape [" + shape.IfcProductLabel.ToString() + "]:"}");
            Console.WriteLine($"{GetIndent(11)}{"-------------------" + vert_indices.Count() + "--------------------"}");
            Console.WriteLine(GetIndent(11)+ "{0}", string.Join(", ", vert_indices));

            /**********************************************************VERT_INDICES_TOXML******************************************************************/
            xmlWriter.WriteStartElement("VertexIndices");
            xmlWriter.WriteAttributeString("CreateGameObject", "False"); //working
            xmlWriter.WriteAttributeString("GenerateMesh", "False"); //trial
            xmlWriter.WriteAttributeString("CountOfVertexIndices", vert_indices.Count.ToString()); //working
            xmlWriter.WriteElementString("V_Indices", string.Format("{0}", string.Join(", ", vert_indices)));
            xmlWriter.WriteEndElement();
        ...
        private static void composetrianglesets(XbimFaceTriangulation face, XbimShapeTriangulation shapemesh, List<int> vertexindices) //works per face
        {
            ...
            //Console.WriteLine("\n" + GetIndent(15) + "Vertex Indices for this Face: " + "{0}", string.Join(", ", face.Indices));
            
            /************************************************************************************************************************/

            for (int i = 0; i < face.TriangleCount; i++) //iterate each traingle of this face of the shapeinstance passed
            {
                string name = "triangle_" + (i).ToString(); // trial -  to keep the triangle id asame as in xml file
